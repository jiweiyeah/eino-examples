...
func main() {
	...
	log.Println("--- Client Side ---")
	mcpTools := getMCPTool(ctx)

	log.Println("--- Tool Invocation ---")
	for i, mcpTool := range mcpTools {
		fmt.Println(i, ":")
		log.Printf("Getting info for tool: %d", i)
		...
		log.Printf("Invoking tool with args: %s", `{"operation":"add", "x":1, "y":1}`)
...
func getMCPTool(ctx context.Context) []tool.BaseTool {
	log.Println("Creating MCP SSE client to connect to 'http://localhost:12345/sse'")
