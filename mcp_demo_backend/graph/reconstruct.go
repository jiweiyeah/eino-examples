...
func NewReconstructGraph(ctx context.Context) (compose.Runnable[string, string], error) {
	// 加载 .env 文件
	...
	_ = sg.AddLambdaNode("prepare_classifier_input_and_prompt", compose.InvokableLambda(func(ctx context.Context, input string) ([]*schema.Message, error) {
		...
		messages, err := classifierPrompt.Format(ctx, inputMap)
		if err != nil {
			return nil, fmt.Errorf("格式化分类器提示词失败: %w", err)
		}
		return messages, nil
	}), compose.WithStatePreHandler(func(ctx context.Context, out string, state *RewriteState) (string, error) {
	...
	_ = sg.AddChatModelNode("classifier_model_stream", chatModel, compose.WithNodeName("分类器模型(流式)"), compose.WithStatePostHandler(
		func(ctx context.Context, out *schema.Message, state *RewriteState) (*schema.Message, error) {
